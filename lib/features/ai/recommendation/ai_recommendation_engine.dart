...
class AiRecommendationEngine {
  static List<AIRecommendationResult> recommendProviders({
    ...
    final req = request is RecommendationRequest
        ? request
    ...
    final keyword =
        (req.requestedServiceKeyword.isNotEmpty
    ...
    for (final p in availableProviders) {
      ...
      if (spec.isNotEmpty) {
        reasons.add('Matches specialty "${p.specialization}".');
      }
      if (p.overallRating >= 4.2) {
        reasons.add('Highly rated (${p.overallRating.toStringAsFixed(1)}/5).');
      }
      if ((p.experienceYears ?? 0) >= 8) {
        reasons.add('Experienced (~${p.experienceYears} yrs).');
      }
      if (ms >= 0.85) reasons.add('Strong medical-profile compatibility.');
      if (reasons.isNotEmpty) {
        reasons.insert(
          0,
          '${p.fullName} is recommended because they are a ${p.specialization}.',
        );
      }
  ...
  static double _specializationScore(String keyword, String spec, String blob) {
    ...
    if (spec.isNotEmpty &&
        blob.contains(spec.substring(0, spec.length.clamp(1, 4)))) {
      return 0.5;
    }
    return 0.2;
  }
  ...
  static double _medicalCompatibilityScore(String blob, ProviderModel p) {
    ...
    for (final tag in p.serviceType.split(',')) {
      if (tag.trim().isNotEmpty && blob.contains(tag.trim().toLowerCase())) {
        score += 0.2;
      }
    }
