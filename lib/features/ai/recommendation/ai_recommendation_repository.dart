...
class PatientRecommendationProfileRepository {
  ...
  Future<PatientRecommendationProfile> _mergeWithApi(
    ...
    var mergedPrevRatings = Map<String, double>.from(
      seed.previousProviderRatings,
    );
    var specializationAffinity = Map<String, double>.from(
      seed.specializationRatingAffinity,
    );
    try {
    ...
    final historyWeighting =
        seed.hasHistoryForWeighting ||
        _hasHistorySignals(clinical) ||
        mergedPrevRatings.isNotEmpty ||
        specializationAffinity.isNotEmpty;

    // Extract only successfully processed AI data from patient uploads.
    final processedAiRecords = clinical
        .where(_isProcessedAiRecordWithUsefulContent)
        .toList(growable: false);
    final aiSummaries = processedAiRecords
        .map(
          (r) => _firstText(r, const [
            'ai_summary',
            'aiSummary',
            'medical_summary',
            'medicalSummary',
        ...
        .toList();
    final ocrTexts = processedAiRecords
        .map(
          (r) => _firstText(r, const [
            'ocr_text',
            'ocrText',
        ...
        .where((s) => s.trim().isNotEmpty)
        .toList();
    final analysisTags = processedAiRecords.expand<String>((r) {
      final raw = r['analysis_tags'] ?? r['analysisTags'] ?? r['tags'];
      if (raw == null) return const <String>[];
      if (raw is List) return raw.map((t) => t.toString());
      try {
        final decoded = jsonDecode(raw.toString());
        if (decoded is List) return decoded.map((t) => t.toString());
      } catch (_) {}
      return const <String>[];
    }).toList();
    ...
    return PatientRecommendationProfile(
      ...
      medications:
          _splitField(profile['currentMedications']) ?? seed.medications,
      previousSurgeries:
  ...
  bool _isProcessedAiRecordWithUsefulContent(Map<String, dynamic> record) {
    final status =
        (record['aiStatus'] ??
                record['ai_status'] ??
                record['extracted_text_status'] ??
                record['extractedTextStatus'] ??
                '')
            .toString()
            .toLowerCase()
            .trim();
    if (status != 'processed') return false;
    final usefulText = _firstText(record, const [
      'ai_summary',
      'aiSummary',
      'medical_summary',
      'medicalSummary',
    ...
    return usefulText.trim().isNotEmpty;
  }
...
class AiMedicalRecordLocalStore {
  ...
  Future<List<MedicalRecordEntry>> load(String patientId) async {
    ...
    try {
      ...
      return list
          .whereType<Map>()
          .map((e) => _entryFromMap(e, patientId))
          .toList();
    } catch (_) {
      return [];
    }
  ...
  MedicalRecordEntry _entryFromMap(Map<dynamic, dynamic> e, String patientId) {
    return MedicalRecordEntry(
      ...
      fileSize: e['fileSize'] is int
          ? e['fileSize'] as int
          : int.tryParse(e['fileSize']?.toString() ?? ''),
      ...
      tags:
          (e['tags'] as List<dynamic>?)?.map((x) => x.toString()).toList() ??
          const [],
